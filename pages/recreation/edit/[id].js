...
export default function EditBusiness() {
    ...
    useEffect(() => {
        const fetchTown = () => {
            axios
            .get(api + '/recreation/' + id)
            .then((res) => {
                setData(res.data)
                setImagePreview(api + '/cdn/recreation/' + res.data.coverImg)
                setImage(res.data.coverImg)
                setImagePreview2(api + '/cdn/recreation/' + res.data.mainImg)
    ...
    return (
        <>
            <Head>
                <title>{"Recreation | " + data.name}</title>
            </Head>
                ...
                <form onSubmit={submitHandler} encType='multipart/form-data'>
                    ...
                    <div className={styles.title}>
                        ...
                        <div className={styles.inputs}>
                            <div>
                                <input type="text" id='name' placeholder="Name" defaultValue={data.name} />
                                <input type="link" id='link' placeholder="Link" defaultValue={data.url} />
                                <input type="text" id='address' placeholder="Address" defaultValue={data.mapsAddress} />
                                <div>
                                    {data.socials.map((data, i) =>
                                        <div className={styles.socialCont} key={i}>
                                            ...
                                            <input id={"user" + i} placeholder="Link" defaultValue={data.user} />
                                        </div>
                                    )}
                                </div>
                            </div>
                            <div>
                                <textarea id="history" cols={23} placeholder="History" defaultValue={data.history} />
                                <textarea id="activities" cols={23} placeholder="Activities" defaultValue={data.activities} />
                            </div>
                        </div>
